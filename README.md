...
扫整个项目文件夹的 Markdown · 排序跟着对话走 · 不调模型、不联网
...
- 只读**当前会话工作区内**的 `.md` 文件（路径越出工作区一律拒绝）
- 只读**当前会话**的对话事件（用来排序）
- **不发起任何对外网络请求**：客户端只有两处 `fetch`，都指向插件自己的同源路由
- **没有安装期脚本**（无 `install` / `postinstall`）
- **零依赖** —— 装完不需要构建授权，也没有构建步骤
- 按路径读文件的接口只放行**图片扩展名白名单**，响应带 `nosniff` 与 `default-src 'none'; sandbox`
...
npm test          # 105 项，零依赖，不需要先 npm install
```
