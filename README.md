...
**关于那行「按「xxx」排序」**：显示的是**语料里真实存在**的词。
候选里总有一些碎片匹配不上任何文档，拿它们当话题会显示成乱码。
...
- **中文分词是 n-gram 近似**：没有引入分词库（那会带来依赖）。跨词边界的碎片已经按
  「首尾是虚词」丢掉了大部分，但**仍可能有残余**（比如「面板里」这种），
  它们会出现在「按「xxx」排序」那行里 —— 匹配不上任何文档的碎片不参与打分
...
npm test          # 206 项，零依赖，不需要先 npm install
```
