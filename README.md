...
![排序跟着对话走：发一句话，右侧栏的文档列表按这句话重排](https://raw.githubusercontent.com/PolinniZhong/dsh-knit/main/docs/demo-reorder.gif)

*排序跟着对话走（同一个工作区、同一个会话）：先问「排序算法用的 BM25 是怎么加权的？」，*
*顶上换成排序那几篇；再问「插件市场收录和下载量怎么查？」，顶上又换成市场调研那几篇。*
*⚠️ 面板是**每 5 秒轮询一次**的，所以重排落在发消息之后的 **0–5 秒**内、不是瞬时；动图比真实时间快。*
